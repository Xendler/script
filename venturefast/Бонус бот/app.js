...
function startTimer() {
    ...
    timer = setInterval(function () {
		...
		if(time ==10)
		{
			requestify.get('http://venturefast.ru/godeposit', {
        secretKey: config.secretKey
    })
        .then(function(response) {
    
            console.tag('[VENTUREFAST.RU]--').log('Бонус бот сделал депозит');
			
        },function(response){
            console.tag('[VENTUREFAST.RU]--').log('Бонус бот не сделал депозит');
