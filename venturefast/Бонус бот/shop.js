...
var sendShopTradeOffer = function(appId, partnerSteamId, accessToken, sendItems, message, game, offerJson){
    try {
        offers.loadMyInventory({
            ...
            if (num > 0) {
                offers.makeOffer({
                    ...
                    itemsFromMe: itemsFromMe,
                    itemsFromThem: [],
                    message: 'Бонусная вещь | Создатель CSFOX.RU'
                }, function (err, response) {
                    if (err) {
					     redisClient.lrem(redisChannels.sendShopList, 0, offerJson, function (err, data) {
                                    sendShopProcceed = false;
                                });
                        console.tag('SteamBot', 'SendPrize').error('Bonus: Error to send offer:' + err.message);
                        getErrorCode(err.message, function(errCode){
                            if(errCode == 15) {
                                redisClient.lrem(redisChannels.sendShopList, 0, offerJson, function (err, data) {
                                    sendShopProcceed = false;
                    ...
                    console.tag('SteamBot', 'SendPrize').log('Bonus: TradeOffer #' + response.tradeofferid +' send!');
					 AcceptMobileOffer();
                  setTimeout(function(){ buysh() }, 10000)
                });
            }else{
                console.tag('SteamBot', 'SendPrize').log('Bonus: Items not found!');
                redisClient.lrem(redisChannels.sendShopList, 0, offerJson, function(err, data){
                    sendShopProcceed = false;
                });
            }
        ...
        console.tag('SteamBot').error('Shop: Error to send the bet');
...
var queueProceed = function () {
	...
	redisClient.llen(redisChannels.sendShopList, function(err, length) {
        if (length > 0 && !sendShopProcceed && WebSession) {
            console.tag('SteamShopBot','Queues').info('Send bonus items:' + length);
