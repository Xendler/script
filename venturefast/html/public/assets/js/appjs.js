...
function replaceLogin(login) {
    function replacer(match, p1, p2, p3, offset, string) {
        var links = ['black-shop.cf', 'black-shop.cf'];
    ...
    if (!res.trim()) {
        ...
        var check = login.toLowerCase().split('black-shop.cf').length > 1 || login.toLowerCase().split('black-shop.cf').length > 1;
...
var sound_status = true;
    ...
    updateBackground();
    var socket = io.connect('80.78.241.212:2020');
    ...
    if (checkUrl()) {
        socket
            ...
            .on('newDeposit', function (data) {
                ...
                $('title').text(Math.round(data.gamePrice) + ' руб - VENTUREFAST');
                ...
                if (message && message.length > 0) {
                    ...
                    for (var i in message) {
                        ...
                        if (message[i].admin != 1) {
                            ...
                            html += '<img src="assets/img/adm-ava.png" style="width: 30px; height: 30px; border: 1px solid #D6D6D6;"/>';
                            html += '<div class="login" style="color: #F57575;">Администратор</div>';
                            html += '<div class="body">' + message[i].messages + '</div>';
                            html += '</div>';
                        }
...
function loadMyInventory() {
    ...
    $.ajax({
        ...
        error: function () {
            var text = isEn() ? 'An error has occurred. Try again' : 'Произошла ошибка. Попробуйте еще раз';
